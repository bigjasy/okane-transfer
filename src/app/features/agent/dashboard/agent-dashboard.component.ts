...
<section class="page"><div class="page-header"><div><h1 class="page-title">Dashboard Agent</h1><p class="page-subtitle">Vue synthétique conforme aux besoins OkaneTransfer.</p></div></div>
...
export class AgentDashboardComponent implements OnInit {
 ...
 ngOnInit(): void { this.data.dashboard('agent').subscribe(s => { this.summary=s; this.lineData={labels:['Jan','Fév','Mar','Avr','Mai','Juin'],datasets:[{label:'Transferts',data:s.charts?.months ?? [10,20,30,40,50,60]}]}; this.pieData={labels:['AVAILABLE','PAID','PENDING','CANCELLED'],datasets:[{data:s.charts?.status ?? [1,1,1,1]}]}; }); }
}
